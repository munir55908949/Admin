...
{
    [AuthorizeUsers]
    public class GuaranteeSortController : ParentController
    {
        // GET: GuaranteeSort
        public ActionResult Index()
        {
            GuaranteeSortModel model = new GuaranteeSortModel();
    ...
    int jtPageSize = 10, string jtSorting = null)
        {
            ...
            var list = _GuaranteeSortServices.SearchGrid(Id, Name, Note, CreatedAt, CreatedBy, out count, jtStartIndex, jtPageSize, jtSorting);
            return Json(new { Result = "OK", Records = list, TotalRecordCount = count });
        }
        public ActionResult Read([DataSourceRequest] DataSourceRequest request)
        {
            return Json(_GuaranteeSortServices.SearchGrid().ToDataSourceResult(request), JsonRequestBehavior.AllowGet);
        }
        ...
        {
            #region prepareData
            var list = _GuaranteeSortServices.SearchGrid().Where(x =>
             (Id > 0 ? x.Id == Id : Id == 0)
        ...
        {
            GuaranteeSortModel model = new GuaranteeSortModel();
            GuaranteeSort obj = new GuaranteeSort();
            if (Id > 0)
            {
                obj = _GuaranteeSortServices.GetGuaranteeSortById(Id.Value);
            }
        ...
        [HttpPost]
        public ActionResult AddEdit(GuaranteeSortModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.Id == 0)//not exist in DB and create new recored
                {
                    ...
                    context.GuaranteeSort.Add(obj);
                }
                context.SaveChanges();
                TempData["Success"] = "تم الحفظ بنجاح";
                return RedirectToAction("Index", "GuaranteeSort");
            }
            InitalizeModelLookups.InitModel(model);
            return View(model);
        }
        ...
        {
            var item = context.GuaranteeSort
