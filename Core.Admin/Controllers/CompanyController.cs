...
{
    [AuthorizeUsers]
    public class CompanyController : ParentController
    {
        // GET: Company
        ...
        public JsonResult Search( int? Id, string Name,string Address, DateTime? CreatedAt, string CreatedBy,
    ...
    int jtPageSize = 10, string jtSorting = null)
        {
            ...
            var list = _CompanyServices.SearchGrid(Id, Name, Address, CreatedAt, CreatedBy, out count, jtStartIndex, jtPageSize, jtSorting);
            return Json(new { Result = "OK", Records = list, TotalRecordCount = count });
        }
        public ActionResult Read([DataSourceRequest] DataSourceRequest request)
        {
            return Json(_CompanyServices.SearchGrid().ToDataSourceResult(request), JsonRequestBehavior.AllowGet);
        }
        public JsonResult ExportReport(int? Id, string Name, string Address,
            DateTime? CreatedAt,  string CreatedBy)
        {
            var path = ExportPdf(Id, Name, Address, CreatedAt, CreatedBy, context);
            if (!string.IsNullOrEmpty(path))
                return Json(path, JsonRequestBehavior.AllowGet);
            else
                return Json(new { error = "حدث خطأ يرجى إعادة المحاولة" });
        }
        public string ExportPdf( int? Id, string Name, string Address,
            DateTime? CreatedAt,  string CreatedBy, DatabseEntities context)
        {
            #region prepareData
            var list = _CompanyServices.SearchGrid().Where(x =>
             (Id > 0 ? x.Id == Id : Id == 0)
            ...
            && (!string.IsNullOrEmpty(Address) ? x.Address.Contains(Address) : string.IsNullOrEmpty(Address))
            ).ToList();
            #endregion
            ...
            using (MemoryStream stream = new System.IO.MemoryStream())
            {
                ...
                headerTable.AddCell(new PdfPCell(new Phrase($"تقـريــر  الشــــركات ", f3))
                {
                ...
                tableRequestDetails.AddCell(new PdfPCell(new Phrase(" الشــــركة", f1)) { BackgroundColor = WebColors.GetRGBColor("#e8f0fe"), MinimumHeight = 30, HorizontalAlignment = Element.ALIGN_CENTER });
                tableRequestDetails.AddCell(new PdfPCell(new Phrase(" عنــوان الشـــركة", f1)) { BackgroundColor = WebColors.GetRGBColor("#e8f0fe"), MinimumHeight = 30, HorizontalAlignment = Element.ALIGN_CENTER });
                tableRequestDetails.AddCell(new PdfPCell(new Phrase("تاريخ تسجيل الشـركة", f1)) { BackgroundColor = WebColors.GetRGBColor("#e8f0fe"), MinimumHeight = 30, HorizontalAlignment = Element.ALIGN_CENTER });
                #endregion
                int index = 1;
                foreach (var model in list)
                {
                    ...
                    tableRequestDetails.AddCell(new PdfPCell(new Phrase(model.Address, f)) { MinimumHeight = 30, HorizontalAlignment = Element.ALIGN_CENTER });
                ...
                secondTable
                    .AddCell(new PdfPCell(new Phrase($"اجمـــالي عــدد الشــــركات /  {list.Count.ToString()} شـــركة", f1))
        ...
        public ActionResult AddEdit(int? Id)
        {
            ...
            if (Id > 0)
            {
                obj = _CompanyServices.GetCompanyById(Id.Value);
            }
        ...
        [HttpPost]
        public ActionResult AddEdit(CompanyModel model)
        {
