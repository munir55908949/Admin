...
{
    [AuthorizeUsers]
    public class SectorController : ParentController
    {
        // GET: Sector
    ...
    int jtPageSize = 10, string jtSorting = null)
        {
            ...
            var list = _SectorServices.SearchGrid(Id, Name, Note, CreatedAt, CreatedBy, out count, jtStartIndex, jtPageSize, jtSorting);
            return Json(new { Result = "OK", Records = list, TotalRecordCount = count });
        }
        public ActionResult Read([DataSourceRequest] DataSourceRequest request)
        {
            return Json(_SectorServices.SearchGrid().ToDataSourceResult(request), JsonRequestBehavior.AllowGet);
        }
        ...
        {
            #region prepareData
            var list = _SectorServices.SearchGrid().Where(x =>
             (Id > 0 ? x.Id == Id : Id == 0)
        ...
        public ActionResult AddEdit(int? Id)
        {
            ...
            if (Id > 0)
            {
                obj = _SectorServices.GetSectorById(Id.Value);
            }
        ...
        [HttpPost]
        public ActionResult AddEdit(SectorModel model)
        {
