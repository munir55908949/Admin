...
{
    [AuthorizeUsers]
    public class ContractDataController : ParentController
    {
        // GET: ContractData
        public ActionResult Index()
        {
            ContractDataModel model = new ContractDataModel();
        ...
        public JsonResult Search(string ContractNumber, int? CompanyId, int? BankId,int? SectorId,int? AdministrationId, int? ContractStatusId, string Note, string CreatedBy,out int AllListCount, int jtStartIndex = 0,int jtPageSize = 10, string jtSorting = null)
        {
            int count = 0;
            AllListCount = 0;
            //pass from controller to service
            var list = _ContractDataServices.SearchGrid(ContractNumber, CompanyId, BankId, SectorId, AdministrationId, ContractStatusId, Note, CreatedBy, out count, jtStartIndex, jtPageSize, jtSorting);
        ...
        {
            return Json(_ContractDataServices.SearchGrid().ToDataSourceResult(request), JsonRequestBehavior.AllowGet);
        }
        [HttpPost]
        public JsonResult ExportReport(string ContractNumber, int? CompanyId, int? BankId, int? SectorId, int? AdministrationId, int? ContractStatusId, string Note, string CreatedBy)

        {
            var path = ExportPdf(ContractNumber, CompanyId, BankId, SectorId, AdministrationId, ContractStatusId, Note, CreatedBy, context);
            if (!string.IsNullOrEmpty(path))
                return Json(path, JsonRequestBehavior.AllowGet);
            else
                return Json(new { error = "حدث خطأ يرجى إعادة المحاولة" });
        }
        public string ExportPdf(string ContractNumber, int? CompanyId, int? BankId, int? SectorId, int? AdministrationId, int? ContractStatusId, string Note, string CreatedBy, DatabseEntities context)
        {
            #region prepareData
            var list = _ContractDataServices.SearchGrid().Where(x =>
            (!string.IsNullOrEmpty(ContractNumber) ? x.ContractNumber.Contains(ContractNumber) : string.IsNullOrEmpty(ContractNumber))
            && (ContractStatusId > 0 ? x.ContractStatusId == ContractStatusId : ContractStatusId == 0)
            && (CompanyId > 0 ? x.CompanyId == CompanyId : CompanyId == 0)
            && (BankId > 0 ? x.BankId == BankId : BankId == 0)
            && (SectorId > 0 ? x.SectorId == SectorId : SectorId == 0)
            && (AdministrationId > 0 ? x.AdministrationId == AdministrationId : AdministrationId == 0)
            ...
            using (MemoryStream stream = new System.IO.MemoryStream())
            {
                ...
                headerTable.AddCell(new PdfPCell(new Phrase($"تقـريــر بيانـــات العقـــــد ", f3))
                {
                ...
                #region //First block 
                PdfPTable tableRequestDetails = new PdfPTable(9); // a table with 4 cell 
                ...
                tableRequestDetails.SetTotalWidth(new float[] {  5, 8, 5, 7, 5, 5, 3 , 5, 3 });
                ...
                tableRequestDetails.AddCell(new PdfPCell(new Phrase("حالة العقد", f)) { BackgroundColor = WebColors.GetRGBColor("#e8f0fe"), MinimumHeight = 30, HorizontalAlignment = Element.ALIGN_CENTER });
                tableRequestDetails.AddCell(new PdfPCell(new Phrase("اسم الشركة", f)) { BackgroundColor = WebColors.GetRGBColor("#e8f0fe"), MinimumHeight = 30, HorizontalAlignment = Element.ALIGN_CENTER });
               tableRequestDetails.AddCell(new PdfPCell(new Phrase("اسم البنك", f)) { BackgroundColor = WebColors.GetRGBColor("#e8f0fe"), MinimumHeight = 30, HorizontalAlignment = Element.ALIGN_CENTER });
                ...
                tableRequestDetails.AddCell(new PdfPCell(new Phrase("اسم الجهة", f)) { BackgroundColor = WebColors.GetRGBColor("#e8f0fe"), MinimumHeight = 30, HorizontalAlignment = Element.ALIGN_CENTER });
                ...
                foreach (var model in list)
                {
                    ...
                    tableRequestDetails.AddCell(new PdfPCell(new Phrase(model.ContractStatusName, f)) { MinimumHeight = 30, HorizontalAlignment = Element.ALIGN_CENTER });
                    tableRequestDetails.AddCell(new PdfPCell(new Phrase(model.CompanyName, f)) { MinimumHeight = 30, HorizontalAlignment = Element.ALIGN_CENTER });
                    tableRequestDetails.AddCell(new PdfPCell(new Phrase(model.BankName, f)) { MinimumHeight = 30, HorizontalAlignment = Element.ALIGN_CENTER });
                    ...
                    tableRequestDetails.AddCell(new PdfPCell(new Phrase(model.AdministrationName, f)) { MinimumHeight = 30, HorizontalAlignment = Element.ALIGN_CENTER });
                ...
                secondTable
                    .AddCell(new PdfPCell(new Phrase($"اجمـالي عــدد العقــود /  {list.Count.ToString()} عقـــد", f1))
        ...
        {
            ContractDataModel model = new ContractDataModel();
            ContractData obj = new ContractData();
            if (Id > 0)
            {
                obj = _ContractDataServices.GetContractDataById(Id.Value);
            }
        ...
        [HttpPost]
        public ActionResult AddEdit(ContractDataModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.Id == 0)//not exist in DB and create new recored
                {
                    ...
                    context.ContractData.Add(obj);
                }
                context.SaveChanges();
                TempData["Success"] = "تم الحفظ بنجاح";
                return RedirectToAction("Index", "ContractData");
            }
